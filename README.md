...
**生成** — 支持多种数据类型（URL / 纯文本 / vCard 联系人 / WiFi 凭证），丰富样式自定义选项。

**识别** — 上传或粘贴二维码图片，Canvas + jsQR 解码识别内容：

- **拖拽上传** — 拖拽包含二维码的截图或照片到页面
- **点击选择** — 点击区域选择本地图片文件
- **Ctrl+V 粘贴** — 直接粘贴剪贴板中的二维码图片
- **自动分类** — 识别结果自动判断类型：URL（可点击跳转）、vCard 联系人（解析姓名/电话/邮箱）、WiFi 凭证（解析 SSID/密码/加密）、纯文本
- **一键复制** — 复制原始识别内容到剪贴板
